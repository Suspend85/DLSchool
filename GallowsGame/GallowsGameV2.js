...
var getGuess = function () {
  ...
  let quess = prompt('Угадайте букву или нажмите Отмена для выхода из игры.');
  return quess;
  };

var updateGameState = function (quess, word, answerArr) {
  // Обновляет answerArr согласно ответу игрока (quess) возвращает число, обозначающее, сколько раз буква quess стречается в слове, чтобы можно было обновить значение remainingLetters
  let counter = 0;
  for (let i = 0; i < word.length; i++) {  //обновляем состояние игры
    if (word[i] === quess.toLowerCase() && answerArr[i] === '_') {   // если ответ совпадает с какой-нибудь из букв
...
var quess = getGuess();
  if (quess === null) {
    ...
    var correctGuesses = updateGameState(quess, word, answerArr);
